...
public class DateChecker extends NodeChecker {
	...
	private CheckStatus checkDateValue(final Element e) {
		...
		try {
			...
			if ( year>s_now_year ) return new CheckStatus("YEAR is in the future");
		...
		try {
			...
			if ( day<0 ) return new CheckStatus("DAY is negative");
			if ( day>31 ) return new CheckStatus("DAY is greater than 31");
