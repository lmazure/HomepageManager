...
public class Parser2 {
	 ...
	 * @param authorFactory
	 */
	public Parser2(final ArticleFactory2 articleFactory,
			      final LinkFactory2 linkFactory,
	...
	private void extractArticles(final Document document,
		...
		for (int i=0; i<list.getLength(); i++) {
            ...
            final Article2 article = a_articleFactory.buildArticle(file, dateYear, dateMonth, dateDay);
			...
			for (int j=0; j<linkNodes.getLength(); j++) {
                ...
                final ParserLinkDto2 linkDto = parseLinkNode(linkNode);
	            final Link2 link = a_linkFactory.newLink(article, linkDto);
			...
			for (int j=0; j<authorNodes.getLength(); j++) {
    			...
    			final ParserAuthorDto2 authorDto = parseLinkAuthor(authorNode);

            	final Author2 author = a_authorFactory.buildAuthor(authorDto);
    ...
    private void extractPersonLinks(final Document document,
        ...
        for (int i=0; i<list.getLength(); i++) {
            ...
            final ParserAuthorDto2 authorDto = parseLinkAuthor((Element)authorNode);

            final Author2 author = a_authorFactory.peekAuthor(authorDto);
            ...
            for (int j = 0; j < clistNode.getElementsByTagName("ITEM").getLength(); j++) {
                ...
                final ParserLinkDto2 linkDto = parseLinkNode(linkNode);
                final Link2 link = a_linkFactory.newLink(null, linkDto);
    ...
    private ParserAuthorDto2 parseLinkAuthor(final Element authorNode) {
        ...
        final ParserAuthorDto2 authorDto = new ParserAuthorDto2(namePrefix, firstName, middleName, lastName, nameSuffix, givenName);
        return authorDto;
    }

    private ParserLinkDto2 parseLinkNode(final Element linkNode) {
        ...
        return new ParserLinkDto2(title, subtitle, url, languages, formats, durationHour, durationMinute, durationSecond, status, protection);
    }
